...
module.exports = Task.extend({
  ...
  run: function runTask(passedArguments, passedFlags) {

    var options = this._options = {
      name: passedArguments[0],
      dest: path.resolve(passedArguments[1] || './' + passedArguments[0])
    };

    assert(options.name, 'You must provide [name] for this command');
    ...
    this._ensureEmptyFolder(options.dest)
      ...
      .then(this._deleteGitFolder.bind(this))
      .then(this._initGit.bind(this))
      .then(this._npmInstall.bind(this))
