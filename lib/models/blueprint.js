...
Blueprint.prototype._mapFileTokens = function mapFileTokens(options) {
  ...
  var userTokens = _.isFunction(this.mapFileTokens) ? this.mapFileTokens(options) : this.mapFileTokens;

  if (_.isObject(userTokens)) {
    _.merge(tokens, userTokens);
  }
...
Blueprint.prototype._locals = function mapTemplateVariables(file, options) {
  ...
  var userVariables =  _.isFunction(this.locals) ? this.locals(file, options) : this.locals;

  if (_.isObject(userVariables)) {
    _.merge(variables, userVariables);
  }
...
/**
 * Gets arguments by merging custom ones with default values
 * @method getArguments
 * @returns {Array}
 */
...
/**
 * Gets flags by merging custom ones with default values
 * @method getFlags
 * @returns {Array}
 */
Blueprint.prototype.getFlags = function getFlags() {
  return this._flags.concat(this.flags || []);
};
